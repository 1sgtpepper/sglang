...
Run from the repository root with PYTHONPATH=python:
  ...
  python test/manual/replayssm_retraction.py --case materialize
  python test/manual/replayssm_retraction.py --case matrix

The default asserts continuation preservation and fails on the pinned baseline.
The matrix explicitly classifies baseline failures and checks every control.
...
def make_case(dtype, width, ring_len, carries_mamba):
    ...
    pool = HybridMambaDecodeReqToTokenPool(
        ...
        linear_replayssm_cache_len=ring_len,
    )
    ...
    req = Req(
        "retraction", "", array("q", [1, 2, 3]), SamplingParams(max_new_tokens=100)
    )
...
def verify(req, pool, values, width):
    ...
    for layer in range(2):
        ...
        gdn_replayssm_spec_decode(
            ...
            d_cache=state.replayssm_d[layer],
            k_cache=state.replayssm_k[layer],
            g_cache=state.replayssm_g[layer],
            rawv_cache=state.replayssm_rawv[layer],
            rawk_cache=state.replayssm_rawk[layer],
            beta_cache=None,
            out=out,
            query_start_loc=torch.tensor([0, width], device="cuda", dtype=torch.int32),
            ssm_state_indices=req.kv.mamba_pool_idx.reshape(1),
            replay_indices=torch.tensor(
                [req.kv.req_pool_idx], device="cuda", dtype=torch.int64
            ),
            write_pos=mp.replayssm_spec_write_pos,
            cache_base=mp.replayssm_cache_base,
            is_flush=mp.replayssm_is_flush,
            max_cache_len=mp.linear_replayssm_cache_len,
            max_spec_len=width,
            null_block_id=-1,
            launch_mode="verify",
        )
...
def commit(req, pool, width, accepted):
    ...
    commit_gdn_replayssm_spec(
        ...
        max_cache_len=mp.linear_replayssm_cache_len,
        max_spec_len=width,
        fold_every_commit=mp.mamba_cache.temporal.dtype != torch.float32,
        null_block_id=-1,
    )
...
def roundtrip(req, pool, allocator, materialize, move_row, reuse_state):
    ...
    torch.testing.assert_close(
        kv.k_buffer[0][slots], torch.full_like(kv.k_buffer[0][slots], 0.25)
    )
    torch.testing.assert_close(
        kv.v_buffer[0][slots], torch.full_like(kv.v_buffer[0][slots], 0.75)
    )
    torch.testing.assert_close(
...
def run(
    ...
    for iteration in range(rounds):
        ...
        pending.append(
            int(pool.mamba_pool.replayssm_spec_write_pos[req.kv.req_pool_idx])
        )
        ...
        identities.append(
            roundtrip(req, pool, allocator, materialize, move_row, reuse_state)
        )
        ...
        json.dumps(
            {
                "baseline_failures": sum(
                    not r["preserved"] for r in results if not r["materialize"]
                ),
                "intervention_failures": sum(
                    not r["preserved"] for r in results if r["materialize"]
                ),
            }
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--case", choices=("baseline", "materialize", "matrix"), default="baseline"
    )
    args = parser.parse_args()
    if args.case == "matrix":
        matrix()
    else:
        result = run(torch.float32, 6, 3, args.case == "materialize", 1)
        assert result["preserved"], (
            "Accepted recurrence was lost across CPU backup/restore"
        )
