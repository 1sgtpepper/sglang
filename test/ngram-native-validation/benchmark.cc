...
int main(int argc, char **argv) {
  if (argc != 2)
    throw std::runtime_error("expected variant name");
  constexpr size_t iterations = 2000;
  for (size_t depth : {18, 64}) {
    for (const std::string workload :
         {"static-live", "static-miss", "growth-live", "growth-miss"}) {
      Param p{};
      ...
      Tokens seed;
      for (size_t i = 0; i < (live ? 4 * depth : 7); ++i)
        seed.push_back(1 + i % 7);
      trie.insert(seed.data(), seed.size());
      Tokens request;
      request.reserve(depth + iterations + 1);
      for (size_t i = 0; i < depth; ++i)
        request.push_back(1 + i % 7);
      MatchState state;
      trie.buildRecency(request.data(), depth, request.back(), 3, p, state,
                        depth);
      int64_t query_ns = 0;
      int64_t checksum = 0;
      ...
      for (size_t i = 0; i < iterations; ++i) {
        if (workload.starts_with("growth")) {
          // Disjoint vocabulary changes topology but cannot change query
          // results.
        ...
        const auto result = trie.buildRecency(tail, depth, request.back(), 3, p,
                                              state, request.size());
        ...
        if (i % 64 == 0) {
          MatchState fresh;
          const auto expected = trie.buildRecency(tail, depth, request.back(),
                                                  3, p, fresh, request.size());
      ...
      const auto cycle_ns =
          std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now() -
                                                               cycle_start)
              .count();
      std::cout << argv[1] << ',' << workload << ',' << depth << ','
                << iterations << ',' << query_ns << ',' << cycle_ns << ','
                << checksum << '\n';
    }
  }
}
