...
Result query(Trie &trie, const Param &p, const Tokens &tokens,
             MatchState &state) {
  return p.match_type == "BFS"
             ? trie.buildRecency(tokens.data(), tokens.size(), tokens.back(), 3,
                                 p, state, tokens.size())
             : trie.buildFrequency(tokens.data(), tokens.size(), tokens.back(),
                                   3, p, state, tokens.size());
}
...
int main(int argc, char **argv) {
  ...
  for (const std::string mode : {"BFS", "PROB"}) {
    ...
    for (size_t appended = 0; appended <= 2; ++appended) {
      ...
      require(original.anchors[0].ptr != nullptr &&
                  original.anchors[1].ptr == nullptr,
              "partial miss fixture");
      ...
      insert(trie, distractor);
      require(live.ptr->version == live.version,
              "live anchor was not evicted/reused");

      MatchState fresh_previous;
      query(trie, p, previous, fresh_previous);
      require(fresh_previous.trie_epoch == original.trie_epoch,
              "no reset occurred");
      require(fresh_previous.anchors[0].ptr == live.ptr,
              "live suffix identity stayed fixed");
      require(fresh_previous.anchors[1].ptr != nullptr,
              "old missing suffix now exists");
      ...
      Tokens golden(learned.begin() + 1 + appended,
                    learned.begin() + 5 + appended);
      require(expected.token == golden, "independent longest-context golden");
      require(intervention.token == golden &&
                  intervention.mask == expected.mask,
              "null-only repair did not restore result");
      require((actual.token != golden) == baseline,
              "unexpected baseline/corrected behavior");
      ++interventions;
    }
    ...
    require(actual.token == expected.token && actual.mask == expected.mask,
            "unrelated-growth negative control");
  }
  std::cout << "null-only interventions=" << interventions
            << " negative-controls=2 passed\n";
}
