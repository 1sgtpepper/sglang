...
TORCH_LIBRARY(ngram_validation, m) {
  m.def(
      "verify(Tensor(a!) predicts, Tensor(b!) indices, Tensor(c!) count, Tensor candidates, "
      "Tensor retrieval, Tensor child, Tensor sibling, Tensor target) -> ()",
      &verify_tree_greedy_cpu);
  m.def(
      "reconstruct(Tensor mask, Tensor lengths, Tensor(a!) positions, Tensor(b!) retrieval, "
